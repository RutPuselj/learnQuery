...
describe('Event Bus', () => {
  ...
  describe('methods', () => {
    ...
    describe('method interaction', () => {
      ...
      it('should not remove other callbacks for event handler with off', () => {
        const callbackOne = jasmine.createSpy('callback');
        const callbackTwo = jasmine.createSpy('callback');
        eventBus.on('a', callbackOne);
        eventBus.on('a', callbackTwo);
        eventBus.off('a', callbackOne);
